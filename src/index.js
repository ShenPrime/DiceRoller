  ...
  if (interaction.commandName === 'leaderboard') {
    try {
      ...
      for (const [index, stats] of leaderboards.overallLeaderboard.entries()) {
        ...
        overallField += `${String(index + 1).padStart(4)}    ${user.username.padEnd(13)}${String(stats.total_rolls).padStart(8)}    ${String(stats.total_crits).padStart(7)}    ${String(stats.overall_crit_percentage).padStart(6)}%\n`;
      }
      overallField += '```';
